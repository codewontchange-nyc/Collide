...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!pub || !priv) { console.error("push-send: VAPID keys not configured"); return json({ error: "internal" }, 500); }
    ...
    webpush.setVapidDetails(Deno.env.get("VAPID_SUBJECT") ?? "mailto:hello@collide.app", pub, priv);
