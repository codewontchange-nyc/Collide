...
const SKIN_TONES: string[] = ["", "#f7e0ce", "#f1cdb0", "#e7b48f", "#d89b6e", "#c1814f", "#a66a3c", "#855530", "#6b4326", "#4e2f1c", "#38220f"];
const SKIN_LEVEL: Record<string, number> = {
  "very-fair": 1, "fair": 2, "light": 3, "medium": 5, "tan": 6, "brown": 8, "deep": 10,
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const skinIdx = SKIN_LEVEL[traits.skin_tone] ?? 0;
    if (skinIdx > 0 && SKIN_TONES[skinIdx]) {
      const fs = svg.indexOf('<g transform="translate(531 487)">');
      const fe = svg.indexOf("</g>", fs);
      svg = svg.slice(0, fs) + svg.slice(fs, fe).replace('fill="#fff"', 'fill="' + SKIN_TONES[skinIdx] + '"') + svg.slice(fe);
    }
