 ...
 ev.respondWith((async function(){
  try{
   ...
   var net=fetch(u,{mode:"cors"}).then(function(res){
    if(res&&res.ok){
     c.put(u,res.clone());
     c.keys().then(function(ks){if(ks.length>cap)for(var i=0;i<ks.length-keep;i++)c.delete(ks[i])})}
